    ...
    namespace particles
    {
        namespace particleToGrid
        {
            namespace derivedAttributes
            {
                ...
                {
                    return setup(unit::si_).unit.energy;
                }
                ...
                DINLINE float_X LarmorPower::operator()(T_Particle& particle) const
                {
                    ...
                    const float_X c2 = setup().physicalConstant.speed_of_light * setup().physicalConstant.speed_of_light;

                    const float3_X mom_dt = (mom - mom_mt1) / float_X(setup().delta_t);
                    const float_X el_factor = charge * charge
                        / (float_X(6.0) * PI * EPS0 * c2 * setup().physicalConstant.speed_of_light * mass * mass) * gamma2 * gamma2;
                    const float_X momentumToBetaConvert = float_X(1.0) / (mass * setup().physicalConstant.speed_of_light * gamma);
