                        ...
                        DINLINE float_COLL operator()(Variables const& v) const
                        {
                            ...
                            const float_COLL factor1 = math::abs((v.charge0 * v.charge1))
                                / (4._COLL * pmacc::math::Pi<float_COLL>::value
                                   * static_cast<float_COLL>(EPS0 * setup().physicalConstant.speed_of_light * setup().physicalConstant.speed_of_light));
                            ...
                            const float_COLL factor3
                                = (v.coeff0 * v.coeff1 / v.comsMomentum0Norm2
                                       * static_cast<float_COLL>(setup().physicalConstant.speed_of_light * setup().physicalConstant.speed_of_light)
                                   + 1._COLL);
                            /*
