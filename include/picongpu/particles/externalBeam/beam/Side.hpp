    ...
    namespace particles
    {
        namespace externalBeam
        {
            namespace beam
            {
                ...
                struct ZRSide
                {
                    static constexpr float_X beamStartPosition[3] = {0.5, 0.5, 1.0};
                    static constexpr unsigned axisMap[3] = {1, 0, 2};
                    static constexpr bool reverse[3] = {true, true, true};
                };
                ...
                /** Provides runtime and compile-time versions of AxisSwap
                 *
                ...
                struct ProbingSideCfg
                {
                    ...
                    //! returns an AxisSwap instance for the Side configuration
                    HINLINE static AxisSwap getAxisSwap()
                    {
                        return AxisSwap{
                            Side::axisMap[0],
                            Side::axisMap[1],
                    ...
                    //! provides some of the AxisSwap functionality on compile-time
                    struct AxisSwapCT
                    {
                        // Never used, should it be removed?
                        //! Swap vectors  from picongpu order to beam order (not a rotation, reverse is not applied)
                        ...
                        //! Swap vectors  from beam order to picongpu order (not a rotation, reverse is not applied)
                        template<typename T_CtVec>
                        struct ReverseSwap
                        {
                        private:
                            ...
                            using ReturnVec1 = typename pmacc::math::CT::
                                Assign<ReturnVec0, GetAssignIdx<0>, typename T_CtVec::x>::type;
                            using ReturnVec2 = typename pmacc::math::CT::
                                Assign<ReturnVec1, GetAssignIdx<1>, typename T_CtVec::y>::type;
                            using ReturnVec3 = typename pmacc::math::CT::
                                Assign<ReturnVec2, GetAssignIdx<2>, typename T_CtVec::z>::type;

                        public:
                            using type = ReturnVec3;
                        };

                        //! convert beam index to a picongpu index
                        ...
                        //! convert picongpu index to a beam index
