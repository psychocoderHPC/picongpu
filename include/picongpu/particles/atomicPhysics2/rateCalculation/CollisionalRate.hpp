...
namespace picongpu::particles::atomicPhysics2::rateCalculation
{
    struct CollisionalRate
    {
            ...
            float_X const densityElectrons) const
        {
            ...
            // unit m^2
            constexpr float_64 barn = 1.e-28;
            constexpr float_64 crossSectionReferenceUnit = 1.e6;
            constexpr float_X conversion_Factor_sigma = static_cast<float_X>(
                (barn * crossSectionReferenceUnit) / (picongpu::UNIT_LENGTH * picongpu::UNIT_LENGTH));
            // m^2 / ((m/UNIT_LENGTH)^2) = m^2/m^2 * UNIT_LENGTH^2
            // [UNIT_LENGTH^2] ~ 1.022e-6

            // DeltaE * sigma * rho_e/DeltaE * v_e
            return energyElectronBinWidth * conversion_Factor_sigma * densityElectrons * c_internal
