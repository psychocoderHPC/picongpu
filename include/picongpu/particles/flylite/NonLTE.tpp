        ...
        namespace flylite
        {
                ...
                std::string const& ionSpeciesName)
            {
                ...
                if(!dc.hasId(helperFields::LocalEnergyHistogram::getName("electrons")))
                    dc.consume(makeUnique<helperFields::LocalEnergyHistogram>("electrons", m_avgGridSizeLocal));

                if(!dc.hasId(helperFields::LocalEnergyHistogram::getName("photons")))
                    dc.consume(makeUnique<helperFields::LocalEnergyHistogram>("photons", m_avgGridSizeLocal));

                if(!dc.hasId(helperFields::LocalDensity::getName("electrons")))
                    dc.consume(makeUnique<helperFields::LocalDensity>("electrons", m_avgGridSizeLocal));

                // for each ion species
                if(!dc.hasId(helperFields::LocalRateMatrix::getName(ionSpeciesName)))
                    dc.consume(makeUnique<helperFields::LocalRateMatrix>(ionSpeciesName, m_avgGridSizeLocal));

                if(!dc.hasId(helperFields::LocalDensity::getName(ionSpeciesName)))
                    dc.consume(makeUnique<helperFields::LocalDensity>(ionSpeciesName, m_avgGridSizeLocal));
            }
