...
)
{
    ...
    const float_X ionDensity = ionDensity_norm.x() * particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC);
    ...
    const float_X deflectionProb = ionDensity * c * DELTA_T(base::PIC) * scaledDeflectionDCS;
    ...
    const float_X newEkin = math::max(Ekin - stoppingPower * DELTA_T(base::PIC), float_X(0.0));
    ...
    const float_X emissionProb = photon::WEIGHTING_RATIO * scalingFactor * ionDensity * c * DELTA_T(base::PIC) * this->scaledSpectrumFunctor(Ekin, kappa);
