...
)
{
    ...
    const float_X deflectionProb = ionDensity * c * DELTA_T(units::PIC) * scaledDeflectionDCS;
    ...
    const float_X newEkin = math::max(Ekin - stoppingPower * DELTA_T(units::PIC), float_X(0.0));
    ...
    const float_X emissionProb = photon::WEIGHTING_RATIO * scalingFactor * ionDensity * c * DELTA_T(units::PIC) * this->scaledSpectrumFunctor(Ekin, kappa);
