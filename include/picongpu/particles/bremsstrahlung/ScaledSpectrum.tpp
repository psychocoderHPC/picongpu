...
float_64 ScaledSpectrum::dcs(const float_64 Ekin, const float_64 kappa, const float_64 targetZ) const
{
    ...
    constexpr float_64 bohrRadius = pi * 4.0 * EPS0(base::PIC) * HBAR(base::PIC) * HBAR(base::PIC) /
        (float_64(ELECTRON_MASS(base::PIC)) * ELECTRON_CHARGE(base::PIC) * ELECTRON_CHARGE(base::PIC));
    constexpr float_64 classicalElRadius = float_64(ELECTRON_CHARGE(base::PIC) * ELECTRON_CHARGE(base::PIC)) / (pi * 4.0 * EPS0(base::PIC) * ELECTRON_MASS(base::PIC) * SPEED_OF_LIGHT*SPEED_OF_LIGHT);
    constexpr float_64 fineStructureConstant = float_64(ELECTRON_CHARGE(base::PIC) * ELECTRON_CHARGE(base::PIC)) / (pi * 4.0 * EPS0(base::PIC) * HBAR(base::PIC) * SPEED_OF_LIGHT);
