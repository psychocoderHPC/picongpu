        ...
        namespace boundary
        {
            ...
            struct ReflectThermalIfOutside : public functor::misc::Parametrized<ThermalParameters>
            {
                ...
                HDINLINE ReflectThermalIfOutside()
                    : energy((m_parameters.temperature * UNITCONV_keV_to_Joule) / setup(unit::si_).unit.energy)
                {
                }
                    ...
                    T_Particle& particle)
                {
                    ...
                    if(crossedBoundary[m_parameters.axis])
                    {
                        ...
                        for(uint32_t d = 0; d < simDim; d++)
                        {
                            ...
                            if(crossedBoundary[d] > 0)
                                timeAfterCross = pos[d] * setup().cell[d] / (velBeforeThermal[d] + epsilon);
                            else if(crossedBoundary[d] < 0)
                                timeAfterCross = (1.0_X - pos[d]) * setup().cell[d] / (-velBeforeThermal[d] - epsilon);
