...
namespace acc
{
    ...
    struct PulseFrontTilt : public T_Unitless
    {
        ...
        )
        {
            ...
            float_X const spaceShift_x = SPEED_OF_LIGHT * math::tan( local_tilt_x ) * timeShift / cellSize(units::PIC).y();
    ...
    struct PulseFrontTilt : public pulseFrontTilt::Unitless< T_Params >
    {
        ...
        HINLINE PulseFrontTilt( uint32_t currentStep )
        {
            ...
            float_64 const runTime = DELTA_T(units::PIC) * currentStep - Unitless::laserTimeShift;

            // calculate focus position relative to the laser initialization plane
            float_X const focusPos = Unitless::FOCUS_POS - Unitless::initPlaneY * CELL_HEIGHT(units::PIC);
