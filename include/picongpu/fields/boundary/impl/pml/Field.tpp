...
namespace picongpu::fields::boundary::impl::pml
{
    namespace detail
    {
         ...
         */
        HINLINE int getOuterLayerBoxLinearSize(GridLayout<simDim> const& gridLayout, Thickness const& globalThickness)
        {
            ...
            auto const nonPmlDataSpace
                = gridDataSpace - (globalThickness.getPositiveBorder() + globalThickness.getNegativeBorder());
    ...
    inline float_X& NodeValues::operator[](uint32_t const idx)
    {
    ...
    inline float_X const& NodeValues::operator[](uint32_t const idx) const
    {
        return *(&xy + idx);
    }
...

    {
        ...
        {
            auto const negativeZLayer = Layer{makeIdx(0, 0, 0), makeIdx(gridSize[0], gridSize[1], negativeSize[2])};
        ...
        auto const positiveYLayer
            = Layer{makeIdx(0, positiveBegin[1], negativeSizeZ), makeIdx(gridSize[0], gridSize[1], positiveBeginZ)};
    ...
    template<typename T_Value>
    HDINLINE typename OuterLayerBox<T_Value>::ValueType const& OuterLayerBox<T_Value>::operator()(Idx const& idx) const
    {
    ...
    Field::OuterLayerBoxType Field::getDeviceOuterLayerBox()
    {
        auto const boxWrapper1d
            = pmacc::DataBoxDim1Access<DataBoxType>{getDeviceDataBox(), data->getGridLayout().getDataSpace()};
