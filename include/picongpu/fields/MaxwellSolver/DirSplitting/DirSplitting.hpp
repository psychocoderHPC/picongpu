        ...
        namespace maxwellSolver
        {
            namespace dirSplitting
            {
                ...
                struct ConditionCheck<DirSplitting<T_CurrentInterpolation>, T_Dummy>
                {
                     ...
                     */
                    PMACC_CASSERT_MSG(
                        DirectionSplitting_Set_dX_equal_dt_times_c____check_your_grid_param_file,
                        (SI::SPEED_OF_LIGHT_SI * SI::DELTA_T_SI) == SI::CELL_WIDTH_SI && (sizeof(T_Dummy*) != 0));
                    PMACC_CASSERT_MSG(
                        DirectionSplitting_use_cubic_cells____check_your_grid_param_file,
                        SI::CELL_HEIGHT_SI == SI::CELL_WIDTH_SI && (sizeof(T_Dummy*) != 0));
#if(SIMDIM == DIM3)
                    PMACC_CASSERT_MSG(
                        DirectionSplitting_use_cubic_cells____check_your_grid_param_file,
                        SI::CELL_DEPTH_SI == SI::CELL_WIDTH_SI && (sizeof(T_Dummy*) != 0));
#endif
                };
            } // namespace dirSplitting
            ...
            private:
                ...
                void propagate(CursorE cursorE, CursorB cursorB, GridSize gridSize) const
                {
                    ...
                    foreach(
                        zone::SphericZone<3>(
                            pmacc::math::Size_t<3>(BlockDim::x::value, gridSizeTwisted.y(), gridSizeTwisted.z())),
            ...
            public:
                ...
                void update_beforeCurrent(uint32_t currentStep) const
                {
                    ...
                    auto fieldE_coreBorder = fieldE->getGridBuffer().getDeviceBuffer().cartBuffer().view(
                        GuardDim().toRT(),
                        -GuardDim().toRT());
                    auto fieldB_coreBorder = fieldB->getGridBuffer().getDeviceBuffer().cartBuffer().view(
                        GuardDim().toRT(),
                        -GuardDim().toRT());
                    ...
                    propagate<Orientation_X>(fieldE_coreBorder.origin(), fieldB_coreBorder.origin(), gridSize);
                    ...
                    propagate<Orientation_Y>(fieldE_coreBorder.origin(), fieldB_coreBorder.origin(), gridSize);

                    __setTransactionEvent(fieldE->asyncCommunication(__getTransactionEvent()));
                    __setTransactionEvent(fieldB->asyncCommunication(__getTransactionEvent()));

                    using Orientation_Z = pmacc::math::CT::Int<2, 0, 1>;
                    propagate<Orientation_Z>(fieldE_coreBorder.origin(), fieldB_coreBorder.origin(), gridSize);
