...
namespace picongpu
{
    namespace currentSolver
    {
         ...
         */
        template<typename T_Strategy, bool T_villabuneCorrection>
        struct ZigZagCIC<particles::shapes::CIC, T_Strategy, T_villabuneCorrection, DIM3>
        {
...
#pragma unroll 2
                for(int l = 0; l < 2; ++l)
                {
                    ...
                    atomicOp(
                        ...
                        volume_reci * cellSize.x() * F1(deltaPos.x(), charge)
                            * ((1 - W1(relayPoint.y(), pos[l].y(), y)) * (1 - W1(relayPoint.z(), pos[l].z(), z))
                               + tmpX));
                    atomicOp(
                        acc,
                        ((*j(x, y + 1, z)).x()),
                        volume_reci * cellSize.x() * F1(deltaPos.x(), charge)
                            * ((W1(relayPoint.y(), pos[l].y(), y)) * (1 - W1(relayPoint.z(), pos[l].z(), z)) - tmpX));
                    atomicOp(
                        acc,
                        ((*j(x, y, z + 1)).x()),
                        volume_reci * cellSize.x() * F1(deltaPos.x(), charge)
                            * ((1 - W1(relayPoint.y(), pos[l].y(), y)) * (W1(relayPoint.z(), pos[l].z(), z)) - tmpX));
                    atomicOp(
                        acc,
                        ((*j(x, y + 1, z + 1)).x()),
                        volume_reci * cellSize.x() * F1(deltaPos.x(), charge)
                            * ((W1(relayPoint.y(), pos[l].y(), y)) * (W1(relayPoint.z(), pos[l].z(), z)) + tmpX));
                    // Y
                    atomicOp(
                        acc,
                        ((*j(x, y, z)).y()),
                        volume_reci * cellSize.y() * F1(deltaPos.y(), charge)
                            * ((1 - W1(relayPoint.x(), pos[l].x(), x)) * (1 - W1(relayPoint.z(), pos[l].z(), z))
                               + tmpY));
                    atomicOp(
                        acc,
                        ((*j(x + 1, y, z)).y()),
                        volume_reci * cellSize.y() * F1(deltaPos.y(), charge)
                            * ((W1(relayPoint.x(), pos[l].x(), x)) * (1 - W1(relayPoint.z(), pos[l].z(), z)) - tmpY));
                    atomicOp(
                        acc,
                        ((*j(x, y, z + 1)).y()),
                        volume_reci * cellSize.y() * F1(deltaPos.y(), charge)
                            * ((1 - W1(relayPoint.x(), pos[l].x(), x)) * (W1(relayPoint.z(), pos[l].z(), z)) - tmpY));
                    atomicOp(
                        acc,
                        ((*j(x + 1, y, z + 1)).y()),
                        volume_reci * cellSize.y() * F1(deltaPos.y(), charge)
                            * ((W1(relayPoint.x(), pos[l].x(), x)) * (W1(relayPoint.z(), pos[l].z(), z)) + tmpY));
                    ...
                    atomicOp(
                        ...
                        volume_reci * cellSize.z() * F1(deltaPos.z(), charge)
                            * ((1 - W1(relayPoint.x(), pos[l].x(), x)) * (1 - W1(relayPoint.y(), pos[l].y(), y))
                               + tmpZ));
                    atomicOp(
                        acc,
                        ((*j(x + 1, y, z)).z()),
                        volume_reci * cellSize.z() * F1(deltaPos.z(), charge)
                            * ((W1(relayPoint.x(), pos[l].x(), x)) * (1 - W1(relayPoint.y(), pos[l].y(), y)) - tmpZ));

                    atomicOp(
                        acc,
                        ((*j(x, y + 1, z)).z()),
                        volume_reci * cellSize.z() * F1(deltaPos.z(), charge)
                            * ((1 - W1(relayPoint.x(), pos[l].x(), x)) * (W1(relayPoint.y(), pos[l].y(), y)) - tmpZ));
                    atomicOp(
                        acc,
                        ((*j(x + 1, y + 1, z)).z()),
                        volume_reci * cellSize.z() * F1(deltaPos.z(), charge)
                            * ((W1(relayPoint.x(), pos[l].x(), x)) * (W1(relayPoint.y(), pos[l].y(), y)) + tmpZ));
                }
            }
        ...
        private:
            DINLINE float_X F1(const float_X deltaPosDir, const float_X charge) const
            {
                return charge * deltaPosDir / DELTA_T;
            }

            DINLINE float_X W1(const float_X x_r, const float_X x, const float_X i) const
            {
        ...
        template<typename T_Strategy, bool T_villabuneCorrection>
        struct ZigZagCIC<particles::shapes::CIC, T_Strategy, T_villabuneCorrection, DIM2>
        {
            /* cloud shape: describe the form factor of a posticle
             * assignment shape: integral over the cloud shape (this shape is defined by the user in
             * species.posam for a species)
...
#pragma unroll 2
                for(int l = 0; l < 2; ++l)
                {
                    ...
                    // Z
                    // we can skip Z if we had no motion in Z direction
                }
            }
        ...
        private:
            DINLINE float_X F1(const float_X x_r, const float_X x_1, const float_X charge) const
            {
