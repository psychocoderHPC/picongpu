...
namespace currentSolver
{
    ...
    struct EsirkepovNative
    {
        ...
        )
        {
            ...
            cptCurrent1D(acc, twistVectorFieldAxes<pmacc::math::CT::Int < 1, 2, 0 > >(cursorJ), rotateOrigin < 1, 2, 0 > (line), cellSize(units::PIC).x());
            cptCurrent1D(acc, twistVectorFieldAxes<pmacc::math::CT::Int < 2, 0, 1 > >(cursorJ), rotateOrigin < 2, 0, 1 > (line), cellSize(units::PIC).y());
            cptCurrent1D(acc, cursorJ, line, cellSize(units::PIC).z());
        }
            ...
            for (int i = begin; i < end; ++i)
            {
                for (int j = begin; j < end; ++j)
                {
                    ...
                    for (int k = begin; k < end; ++k)
                    {
                        ...
                        accumulated_J += -this->charge * (float_X(1.0) / float_X(CELL_VOLUME(units::PIC) * DELTA_T(units::PIC))) * W * cellEdgeLength;
