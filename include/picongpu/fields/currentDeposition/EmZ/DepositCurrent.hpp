    ...
    > : public BaseMethods< ParticleAssign >
    {
        ...
        ) const
        {
            ...
            cptCurrent1D(
                ...
                cellSize(units::PIC).x( ) * chargeDensity / DELTA_T(units::PIC)
            );
            cptCurrent1D(
                acc,
                twistVectorFieldAxes< pmacc::math::CT::Int < 2, 0, 1 > >( cursorJ ),
                rotateOrigin< 2, 0, 1 >( line ),
                cellSize(units::PIC).y( ) * chargeDensity / DELTA_T(units::PIC)
            );
            cptCurrent1D(
                acc,
                cursorJ,
                line,
                cellSize(units::PIC).z( ) * chargeDensity / DELTA_T(units::PIC)
            );
        }
