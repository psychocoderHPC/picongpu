    ...
    > : public BaseMethods< ParticleAssign >
    {
        ...
        ) const
        {
            ...
            cptCurrent1D(
                ...
                cellSize(base::PIC).x( ) * chargeDensity / DELTA_T(base::PIC)
            );
            cptCurrent1D(
                acc,
                twistVectorFieldAxes< pmacc::math::CT::Int < 2, 0, 1 > >( cursorJ ),
                rotateOrigin< 2, 0, 1 >( line ),
                cellSize(base::PIC).y( ) * chargeDensity / DELTA_T(base::PIC)
            );
            cptCurrent1D(
                acc,
                cursorJ,
                line,
                cellSize(base::PIC).z( ) * chargeDensity / DELTA_T(base::PIC)
            );
        }
