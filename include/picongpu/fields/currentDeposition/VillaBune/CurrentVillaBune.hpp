...
namespace currentSolver
{
    ...
    struct VillaBune
    {
                                ...
                                const ChargeType charge, const float_X deltaTime)
        {
            ...
            //               dx_inCell = v.x() * dt / cellSize(units::PIC).x()
            const float3_X deltaPos(
                                    velocity.x() * deltaTime / cellSize(units::PIC).x(),
                                    velocity.y() * deltaTime / cellSize(units::PIC).y(),
                                    velocity.z() * deltaTime / cellSize(units::PIC).z());
