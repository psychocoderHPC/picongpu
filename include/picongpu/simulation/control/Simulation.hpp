...
namespace picongpu
{
    ...
    public:
        ...
        void init() override
        {
            ...
            auto alpakaDevice = manager::Device<AccDev>::get().current();
            /* Create an empty allocator. This one is resized after all exchanges
             * for particles are created */
            deviceHeap.reset(new DeviceHeap(alpakaDevice, alpakaQueue, 0u));
    ...
    private:
        ...
        size_t freeDeviceMemory() const
        {
            ...
            std::optional<::alpaka::Buf<AccDev, std::byte, AlpakaDim<1>, size_t>> tmpBuffer{};
            ...
            do
            {
                ...
                {
                    memAlloced = true;
                    tmpBuffer = alpaka::allocBuf<std::byte, size_t>(
                        pmacc::manager::Device<AccDev>::get().current(),
                        allocatableMemory);
                }
                catch(...)
                {
