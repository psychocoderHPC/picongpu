                    ...
                    if( linearIdx < particlesInSuperCell )
                    {
                        ...
                        )
                        {
                            ...
                            float_X const normedWeighting = weighting /
                                float_X( particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC) );
                            cupla::atomicAdd(
                                acc,
                                &( shBin[ binNumber ] ),
                                normedWeighting,
    ...
    void calBinEnergyParticles(uint32_t currentStep)
    {
        ...
        if (writeToFile)
        {
            ...
            for (int i = 0; i < realNumBins; ++i)
            {
                ...
                outFile << std::scientific << (binReduced[i]) * float_64(particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC)) << " ";
            }
            /* endl: Flush any step to the file.
             * Thus, we will have data if the program should crash.
             */
            outFile << std::scientific << count_particles * float_64(particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC))
                << std::endl;
        }
    }
