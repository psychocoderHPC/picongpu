...
struct Div<DIM3, ValueType>
{
    ...
    {
        const ValueType reciWidth = float_X(1.0) / cellSize(base::PIC).x();
        const ValueType reciHeight = float_X(1.0) / cellSize(base::PIC).y();
        const ValueType reciDepth = float_X(1.0) / cellSize(base::PIC).z();
...
void ChargeConservation::notify(uint32_t currentStep)
{
    ...
    *maxChargeDiff_host.origin() = maxChargeDiff;
    ...
    this->output_file << currentStep << " " << (*maxChargeDiff_cluster.origin() * CELL_VOLUME(base::PIC)).x()
