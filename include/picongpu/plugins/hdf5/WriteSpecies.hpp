...
namespace picongpu
{
    namespace hdf5
    {
        ...
        public:
            ...
            HINLINE void operator()(ThreadParams* params, const Space domainOffset)
            {
                ...
                log<picLog::INPUT_OUTPUT>("HDF5:  ( end ) count particles: %1% = %2%") % T_SpeciesFilter::getName()
                    % numParticles;
                ...
                if(numParticles != 0)
                {
                    log<picLog::INPUT_OUTPUT>("HDF5:  (begin) get mapped memory device pointer: %1%")
                    ...
                    log<picLog::INPUT_OUTPUT>("HDF5:  ( end ) get mapped memory device pointer: %1%")
                        % T_SpeciesFilter::getName();

                    log<picLog::INPUT_OUTPUT>("HDF5:  (begin) copy particle to host: %1%")
                        % T_SpeciesFilter::getName();
                    ...
                    log<picLog::INPUT_OUTPUT>("HDF5:  ( end ) copy particle to host: %1%")
                        % T_SpeciesFilter::getName();
                 ...
                 * particles.
                 */
                log<picLog::INPUT_OUTPUT>("HDF5:  (begin) collect particle sizes for %1%")
                    % T_SpeciesFilter::getName();
                ...
                log<picLog::INPUT_OUTPUT>("HDF5:  (begin) write particle records for %1%")
                    % T_SpeciesFilter::getName();
                ...
                writeToHdf5(params, hostFrame, speciesPath, numParticles, numParticlesOffset, numParticlesGlobal);
                ...
                log<picLog::INPUT_OUTPUT>("HDF5:  (begin) writing particlePatches for %1%")
                    % T_SpeciesFilter::getName();
                ...
                {
                    const uint64_t patchOffset = globalDomain.offset[d] + params->window.globalDimensions.offset[d]
                        + params->window.localDimensions.offset[d];
                    const uint64_t patchExtent = params->window.localDimensions.size[d];
                    ...
                    params->dataCollector->write(
                        ...
                        1,
                        myPatchEntries,
                        (particlePatchesPath + std::string("/offset/") + componentNames[d]).c_str(),
                        &patchOffset);
                    params->dataCollector->write(
                        params->currentStep,
                        numPatches,
                        ...
                        1,
                        myPatchEntries,
                        (particlePatchesPath + std::string("/extent/") + componentNames[d]).c_str(),
                        &patchExtent);
                ...
                log<picLog::INPUT_OUTPUT>("HDF5:  ( end ) writing particlePatches for %1%")
                    % T_SpeciesFilter::getName();
        ...
        private:
                ...
                const std::vector<float_64>& unitDimension)
            {
                ...
                params->dataCollector
                    ->writeAttribute(params->currentStep, ctDouble, recordPath.c_str(), "value", &value);
                ...
                params->dataCollector
                    ->writeAttribute(params->currentStep, ctDouble, recordPath.c_str(), "unitSI", &unitSI);
