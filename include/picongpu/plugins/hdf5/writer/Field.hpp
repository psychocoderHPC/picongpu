    ...
    namespace hdf5
    {
        ...
        struct Field
        {
                ...
                const bool isDomainBound)
            {
                ...
                log<picLog::INPUT_OUTPUT>("HDF5 write field: %1% %2%") % name % nComponents;
                ...
                splashGlobalOffsetFile[1]
                    = std::max(0, localDomain.offset[1] - params->window.globalDimensions.offset[1]);
                ...
                if(!isDomainBound)
                {
                    ...
                    uint64_t localSizeInfo[2] = {localSize, rank};
                    ...
                    splashGlobalDomainSize = Dimensions(globalSize, 1, 1);
                    // Offset for this rank
                    splashGlobalOffsetFile = Dimensions(globalOffsetFile, 0, 0);
                ...
                for(uint32_t n = 0; n < nComponents; n++)
                {
                    ...
                    __getTransactionEvent().waitForFinished();
                    params->dataCollector->writeDomain(
                        params->currentStep, /* id == time step */
                ...
                for(uint32_t d = 0; d < simDim; ++d)
                    gridGlobalOffset.at(simDim - 1 - d)
                        = float_64(cellSize[d]) * float_64(splashGlobalDomainOffset[d]);
                params->dataCollector->writeAttribute(
                    params->currentStep,
                    ctDouble,
                    recordName.c_str(),
                ...
                params->dataCollector
                    ->writeAttribute(params->currentStep, ctDouble, recordName.c_str(), "gridUnitSI", &UNIT_LENGTH);
