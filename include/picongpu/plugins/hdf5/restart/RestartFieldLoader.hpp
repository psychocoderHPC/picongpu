        ...
        public:
                ...
                const bool isDomainBound)
            {
                ...
                if(!isDomainBound)
                {
                    ...
                    local_domain_size = Dimensions(elementCount, 1, 1);
                    ...
                    domain_offset = Dimensions(domainOffset, 0, 0);
                ...
                for(uint32_t i = 0; i < numComponents; ++i)
                {
                    ...
                    log<picLog::INPUT_OUTPUT>("Read from domain: offset=%1% size=%2%") % domain_offset.toString()
                        % local_domain_size.toString();
                    DomainCollector::DomDataClass data_class;
                    DataContainer* field_container = params->dataCollector->readDomain(
                        params->currentStep,
                        (std::string("fields/") + objectName + std::string("/") + componentNames[i]).c_str(),
