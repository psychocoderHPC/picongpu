    ...
    namespace hdf5
    {
        ...
        public:
            struct Help : public plugins::multi::IHelp
            {
                ...
                {
                    return std::shared_ptr<ISlave>(new HDF5Writer(help, id, cellDescription));
                }

                plugins::multi::Option<std::string> notifyPeriod = {"period", "enable HDF5 IO [for each n-th step]"};
                plugins::multi::Option<std::string> source = {"source", "data sources: ", "species_all, fields_all"};

                plugins::multi::Option<std::string> fileName = {"file", "HDF5 output filename (prefix)"};
                    ...
                    if(selfRegister)
                    {
                        ...
                        for(auto const& dateSourceNames : source)
                        {
                            auto vectorOfDataSourceNames
                                = plugins::misc::splitString(plugins::misc::removeSpaces(dateSourceNames));
             ...
             */
            HDF5Writer(std::shared_ptr<plugins::multi::IHelp>& help, size_t const id, MappingDesc* cellDescription)
                ...
                const uint32_t restartChunkSize)
            {
                ...
                uint32_t slides = 0;
                mThreadParams.dataCollector->readAttributeInfo(restartStep, nullptr, "sim_slides")
                    .read(&slides, sizeof(slides));
        ...
        private:
            ...
            struct CallWriteFields
            {
                ...
                {
                    bool const containsDataSource
                        = plugins::misc::containsObject(vectorOfDataSourceNames, T_Field::getName());

                    if(containsDataSource)
                    {
                        WriteFields<T_Field> writeFields;
                        writeFields(params);
                    }
                }
            };
            ...
            void writeHDF5(void* p_args)
            {
                ...
                DataSpace<simDim> domainOffset(subGrid.getGlobalDomain().offset + subGrid.getLocalDomain().offset);
                ...
                if(m_help->selfRegister)
                {
                    ...
                    vectorOfDataSourceNames = plugins::misc::splitString(plugins::misc::removeSpaces(dateSourceNames));
                }
                ...
                else
                {
                    ...
                    if(dumpFields)
                    {
                        meta::ForEach<FileOutputFields, WriteFields<bmpl::_1>> forEachWriteFields;
                        forEachWriteFields(threadParams);
                    }

                    meta::ForEach<typename Help::AllFieldSources, CallWriteFields<bmpl::_1>>{}(
                ...
                if(threadParams->isCheckpoint)
                {
                    meta::ForEach<FileCheckpointParticles, WriteSpecies<plugins::misc::UnfilteredSpecies<bmpl::_1>>>
                        writeSpecies;
                ...
                log<picLog::INPUT_OUTPUT>(
                    "HDF5: Writing IdProvider state (StartId: %1%, NextId: %2%, maxNumProc: %3%)")
                    % idProviderState.startId % idProviderState.nextId % idProviderState.maxNumProc;
                WriteNDScalars<uint64_t, uint64_t>()(
                    *threadParams,
                    "picongpu/idProvider/startId",
                    idProviderState.startId,
                    "maxNumProc",
                    idProviderState.maxNumProc);
                WriteNDScalars<uint64_t>()(*threadParams, "picongpu/idProvider/nextId", idProviderState.nextId);
