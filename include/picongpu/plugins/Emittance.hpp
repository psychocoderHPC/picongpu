                ...
                forEachParticle(
                    ...
                    )
                    {
                        ...
                        if( accFilter( acc, particle ) )
                        {
                            ...
                            float_X const normedWeighting = weighting / float_X( particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC) );
                            ...
                            auto const globalCellOffset = globalOffset
                                                        + localSupercellStart
                                                        + frameCellOffset;
                            float_X const posX = ( float_X( globalCellOffset.x( ) ) + pos.x( ) ) * cellSize(base::PIC).x( );
        ...
        void calculateCalcEmittance( uint32_t currentStep )
        {
            ...
            if( writeToFile )
            {
                ...
                if ( currentStep > 0.0 ){
                    ...
                    /* the scaling with normalized weighting (weighting / particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE(base::PIC))
