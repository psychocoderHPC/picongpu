...
namespace openPMD
{
    ...
    struct WriteMeta
    {
        ...
        operator()( ThreadParams * threadParams )
        {
            ...
            iteration.setDt< float_X >( DELTA_T(units::PIC) );
            iteration.setTime( float_X( threadParams->currentStep ) * DELTA_T(units::PIC) );
            ...
            {
                iteration.setAttribute( names[ i ], cellSize(units::PIC)[ i ] );
            }
