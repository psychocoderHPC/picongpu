...
namespace xrayScattering
{
    ...
    **/
    ...
    public:
        ...
        XrayScattering( )
            : pluginName(
            "xrayScattering: Calculate the SAXS scattering intensity of a "
            "species."
        ),
        ...
        void pluginRegisterHelp( po::options_description &desc ) override
        {
            desc.add_options( )(
                ...
                po::value< int >( & numVectors[ 0 ] )
                    ->default_value( 100 ),
                "number of qx"
            )(
                ( pluginPrefix + ".n_qy" ).c_str( ),
                po::value< int >( & numVectors[ 1 ] )
                    ->default_value( 100 ),
                "number of qy"
            )(
                (pluginPrefix + ".file").c_str( ),
                po::value< std::string >( & fileName )
                    ->default_value( pluginName + "Output" ),
                "output file name"
            )(
                ...
                po::value< std::string >( & compressionMethod )
                    ->default_value( "" ),
    ...
    private:
        ...
        void pluginLoad( ) override
        {
            if ( !notifyPeriod.empty( ) )
            {
                ...
                else
                {
                    ...
                    bufferSize = totalNumVectors / countRanks +
                        ( ( totalNumVectors % countRanks ) != 0 );
                    // Number of scattering vectors on the last chunk.
                    resOfVectors = bufferSize - ( bufferSize * countRanks -
                        totalNumVectors);
                    // Initiate the additional amplitude storage for receiving
                    // data and initiate it with zeros.
                    amplitudeReceive.assign(
                        bufferSize,
                        complex_X( 0.0 )
                    );
                }
                ...
                // density).
                using ElectronDensitySolver =
                    typename DetermineElectronDensitySolver<
                    T_ParticlesType >::type;
                // Output unit:
                const float_64 amplitudeUnit = static_cast< float_64 >(
                    FieldTmp::getUnit< ElectronDensitySolver >( )[ 0 ] ) *
                    CELL_WIDTH(base::SI) * CELL_HEIGHT(base::SI) * CELL_DEPTH(base::SI)
                    * ELECTRON_RADIUS_SI;

                // Set the total number of cells in the simulation.
                totalSimulationCells =  Environment< simDim >::get( ).SubGrid(
        ...
        HINLINE uint32_t calcOffset( uint32_t const & step ) const
        {
            ...
            chunk = ( (chunk - ( totalRotations % countRanks ) ) %
                       countRanks );
        ...
        HINLINE void writeOutput( )
        {
            if ( outputLayout == OutputMemoryLayout::Distribute )
            {
                ...
                uint64_t offset = precisionCast< uint64_t >( calcOffset(
                    countRanks - 1 ) );
                uint64_t extent;
                if ( hasLastChunk( countRanks - 1 ) ) extent = resOfVectors;
                else extent =  amplitude->getHostBuffer( ).getCurrentSize( );
                ( * dataWriter )(
                    currentStep,
                    extent,
                    offset,
            ...
            else
            {
                ...
                amplitudeMaster.assign(
                    amplitudeMaster.size( ),
                    complex_X( 0.0 )
                );
            }
        }
         ...
         */
        HINLINE FieldTmp::DataBoxType calculateDensity(
            DataConnector & dc,
        ...
        )
        {
            ...
            auto tmpField = dc.get< FieldTmp >(
                FieldTmp::getUniqueId( 0 ),
                true
            );
            // Initiate with zeros.
            tmpField->getGridBuffer( ).getDeviceBuffer( ).setValue(
                    FieldTmp::ValueType::create( 0.0 ) );
            // Chose species.
            auto species = dc.get< T_ParticlesType >(
            ...
            using ElectronDensitySolver =
                ...
                T_ParticlesType >::type;
            // Calculate density.
            tmpField-> template computeValue<
                CORE + BORDER,
            ...
            FieldTmp::DataBoxType tmpFieldBox =
                tmpField->getGridBuffer( ).getDeviceBuffer( ).getDataBox( );
            return tmpFieldBox;
        }
        ...
        )
        {
            ...
            for( uint32_t step = 0; step < countRanks; step++ )
            {
                ...
                else countVectors = amplitude->getHostBuffer(
                    ).getCurrentSize( );
                // Start the kernel.
                PMACC_KERNEL(
        ...
        HINLINE void notify( uint32_t currentStep ) override
        {
            ...
            const SubGrid< simDim > & subGrid =
                Environment< simDim >::get( ).SubGrid( );
            DataSpace<simDim> globalOffset( subGrid.getLocalDomain( ).offset );
            // Calculate the density and get a data box to access this TmpField.
            DataConnector & dc = Environment<>::get( ).DataConnector( );
            FieldTmp::DataBoxType tmpFieldBox = calculateDensity(
                dc,
                globalOffset
            );
            ...
            const picongpu::traits::FieldPosition<
                typename fields::CellType,
                FieldTmp
            > fieldPos;
            // Shift the density box to exclude the GUARD.
            DataSpace < simDim > guardingSC =
                cellDescription.getGuardingSuperCells( );
            auto const fieldTmpNoGuard =
                tmpFieldBox.shift( guardingSC * SuperCellSize::toRT( ) );
            // Get the field size on this rank (no GUARD).
            DataSpace < simDim > cellsGrid =
                ( cellDescription.getGridSuperCells( )  - 2 * guardingSC ) *
                SuperCellSize::toRT( );
...
namespace traits
{
    ...
    struct SpeciesEligibleForSolver<
        T_Species,
        plugins::xrayScattering::XrayScattering< T_UnspecifiedSpecies >
    >
    {
