    ...
    struct radFormFactor
    {
      ...
      HDINLINE float_X operator()( const float_X N, const float_X omega, vector_X const & observer_unit_vec ) const
      {
          ...
          for( uint32_t d = 0; d < DIM3; ++d )
              sincValue *= pmacc::math::sinc( observer_unit_vec[d] * cellSize(base::PIC)[d] / ( SPEED_OF_LIGHT * float_X( 2.0 ) ) * omega );
...
namespace radFormFactor_CIC_1Dy
{
    struct radFormFactor
    {
      ...
      HDINLINE float_X operator()(const float_X N, const float_X omega, const vector_X observer_unit_vec) const
      {
          return math::sqrt(
              N + ( N * N - N ) * util::square(
                  pmacc::math::sinc( CELL_HEIGHT(base::PIC) / ( SPEED_OF_LIGHT * float_X( 2.0 ) ) * omega )
              )
          );
      }
    };
...
namespace radFormFactor_Gauss_spherical
{
    struct radFormFactor
    {
      ...
      HDINLINE float_X operator()(const float_X N, const float_X omega, const vector_X observer_unit_vec) const
      {
          ...
          return math::sqrt(
              N + ( N * N - N ) * util::square(
                  math::exp( float_X( -0.5 ) * util::square( omega * float_X( 0.5 ) * DELTA_T(base::PIC) ) )
              )
          );
      }
    };
      ...
      HDINLINE float_X operator()(const float_X N, const float_X omega, const vector_X observer_unit_vec) const
      {
        return math::sqrt(
             N + ( N * N - N ) * util::square(
                 math::exp(
                     float_X( -0.5 ) * (
                         util::square( observer_unit_vec.x() * CELL_WIDTH(base::PIC) / ( SPEED_OF_LIGHT * float_X(2.0) ) * omega ) +
                         util::square( observer_unit_vec.y() * CELL_HEIGHT(base::PIC) / ( SPEED_OF_LIGHT * float_X(2.0) ) * omega ) +
                         util::square( observer_unit_vec.z() * CELL_DEPTH(base::PIC) / ( SPEED_OF_LIGHT * float_X(2.0) ) * omega )
                     )
                 )
             )
        );
