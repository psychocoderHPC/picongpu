...
namespace pmacc
{
    namespace nvidia
    {
        ...
        template<typename T_Acc, typename T_Hierarchy>
        DINLINE float atomicAddEmulated(T_Acc const& acc, float* address, float const val, const T_Hierarchy&)
        {
        ...
        template<typename T_Acc, typename T_Hierarchy>
        DINLINE double atomicAddEmulated(T_Acc const& acc, double* address, double const val, const T_Hierarchy&)
        {
            ...
            do
            {
                ...
                old = atomicCAS(
                    ...
                    static_cast<unsigned long long int>(__double_as_longlong(
                        val + __longlong_as_double(static_cast<unsigned long long int>(assumed)))));
                // Note: uses integer comparison to avoid hang in case of NaN (since NaN != NaN)
            } while(assumed != old);
        ...
        DINLINE float atomicAddEmulated(
            T_Acc const& acc,
            T_Type* ptr,
            T_Type const value,
            const T_Hierarchy& hierarchy)
