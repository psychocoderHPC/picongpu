    ...
    public:
            ...
            , hostBuffer(nullptr)
        {
            ...
            if(DIM > DIM1)
            {
                ...
                deviceDoubleBuffer = memory::makeUnique<DeviceBuffer>(tmp_size, false, true);
            }
        ...
        {
            using DeviceBuffer = DeviceBufferIntern<TYPE, DIM>;
            deviceBuffer = memory::makeUnique<DeviceBuffer>(exchangeDataSpace, sizeOnDevice);
            //  this->deviceBuffer = new DeviceBufferIntern<TYPE, DIM > (exchangeDataSpace, sizeOnDevice,true);
            if(DIM > DIM1)
            {
                /*create double buffer on gpu for faster memory transfers*/
                deviceDoubleBuffer = memory::makeUnique<DeviceBuffer>(exchangeDataSpace, false, true);
            }
