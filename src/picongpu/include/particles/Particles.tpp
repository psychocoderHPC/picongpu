...
void Particles<T_ParticleDescription>::update(uint32_t )
{
    ...
    typedef typename GetFlagType<FrameType,pusher<> >::type SearchedPusher;
    /* if no pusher was defined we use PusherNone as fallback */
    typedef typename boost::mpl::if_<hasPusher,SearchedPusher,particlePusher::None >::type SelectPusher;
