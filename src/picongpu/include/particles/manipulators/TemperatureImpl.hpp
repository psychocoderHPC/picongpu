...
template<typename T_ParamClass, typename T_ValueFunctor, typename T_SpeciesName>
struct TemperatureImpl : protected T_ValueFunctor
{
    ...
    DINLINE void operator()(const DataSpace<simDim>& localCellIdx, T_Particle& particle)
    {
        ...
        const float3_X mom = tmpRand * (float_X) math::sqrt(
                                                            precisionCast<sqrt_X > (
                                                                                    macroEnergy * getMass<FrameType>(macroWeighting)
                                                                                    )
                                                            );
