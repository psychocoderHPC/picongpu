...
struct Esirkepov<DIM2,ParticleAssign,NumericalCellType>
{
    ...
    DINLINE void operator()(DataBoxJ dataBoxJ,
                            ...
                            const ChargeType charge, const floatD_X& cellSize, const float_X deltaTime)
    {
        this->charge = charge;
        const floatD_X deltaPos = floatD_X(velocity.x() * deltaTime / cellSize.x(),
                              ...
                              const float_X cellEdgeLength)
    {
        ...
        {
            cptCurrentInLineOfCells(cursorJ(0, y), line - floatD_X(0., y), cellEdgeLength);
        }
    }
                             ...
                             const float_X v_z)
    {
        ...
        for (int x = begin + offset_x; x <= end + offset_x; x++)
        {
            for (int y = begin + offset_y; y <= end + offset_y; y++)
            {
                cptCurrentZInCell(cursorJ(x, y), line - floatD_X(x, y), v_z);
            }
        }
    }
...
struct GetMargin<picongpu::currentSolverEsirkepov::Esirkepov<DIM2,ParticleShape, NumericalCellType> >
{
private:
    typedef picongpu::currentSolverEsirkepov::Esirkepov<DIM2,ParticleShape, NumericalCellType> Solver;
public:
