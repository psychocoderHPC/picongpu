...
public:
    ...
    HINLINE void operator()(ThreadParams* params)
    {
        ...
        if (totalNumParticles != 0)
        {
            ...
            const int iterationsForLoad = ceil(double(totalNumParticles) / double(chunkSize));

            for (int i = 0; i < iterationsForLoad; ++i)
            {
                log<picLog::INPUT_OUTPUT > ("HDF5:   ( begin ) Load particle chunk offset=%1% size=%2%") %
                    (i * chunkSize) % chunkSize;
                __cudaKernel(copySpeciesGlobal2Local)
                    (ceil(double(chunkSize) / double(cellsInSuperCell)), cellsInSuperCell)
                ...
                log<picLog::INPUT_OUTPUT > ("HDF5:   ( end ) Load particle chunk offset=%1% size=%2%") %
                    (i * chunkSize) % chunkSize;
            }
            ...
            /* this cost a little bit of time but hdf5 loading is slower^^
             * not prevent from loading to much particle
             */
