    ...
    char **argv)
{
    ...
    desc.add_options( )
            ...
            ( "steps,s", po::value<std::size_t>(&steps), "simulation steps" )
    ...
    {
        std::cerr << "Invalid or missing grid size." << std::endl << "use -g width height [depth=1]" << std::endl;
        MPI_CHECK(MPI_Finalize());
        return EXIT_FAILURE;
    }

    // after checking all input values, copy into DataSpace Datatype
    PMacc::DataSpace<DIM2> const gpus(devices[0], devices[1]);
    PMacc::DataSpace<DIM2> const grid(gridSize[0], gridSize[1]);
    PMacc::DataSpace<DIM2> const endless(periodic[0], periodic[1]);

    std::size_t const strLen(rule.length());
    std::size_t const gPoint(rule.find('/'));
    std::string const stayAliveIf(rule.substr(0, gPoint));
    std::string const newBornIf(rule.substr(gPoint + 1, strLen - gPoint - 1));

    std::uint32_t ruleMask(0);
    for(std::size_t i(0); i < newBornIf.length(); ++i)
    {
    ...
    for (std::size_t i(0); i < stayAliveIf.length(); ++i)
    {
        std::stringstream ss;
        ss << stayAliveIf[i];
        int shift;
    ...
    std::cout << "newborn: " << newBornIf << " stay alive:" << stayAliveIf << " mask:" << ruleMask << std::endl;
