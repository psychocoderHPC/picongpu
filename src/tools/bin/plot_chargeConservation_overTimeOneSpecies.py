...
def deviation_charge_conservation(series, iteration, timeStep):
    """
    ...
    if is2D:
        ...
        # compute difference between electric field divergence and charge
        # density
        tmp_iter = series.iterations[0]
        print(type(tmp_iter))
        charge_0 = tmp_iter.meshes["e_all_chargeDensity"][io.Mesh_Record_Component.SCALAR][:]
        series.flush()

        diff = (div * EPS0 - (charge[1:, 1:, 1:] - charge_0[1:, 1:, 1:]))
        ...
        for iteration in series.iterations:
            if (iteration >= first_step and
                ...
                cc_max, mean_abs, std, norm = deviation_charge_conservation(
                    series, series.iterations[iteration], iteration)
