...
.TBG_tasks_adjusted_tmp=$((!TBG_nodes_adjusted * !TBG_numHostedDevicesPerNode))
.TBG_tasks_adjusted=$(if [ $TBG_tasks -lt $TBG_tasks_adjusted_tmp ] ; then echo $TBG_tasks; else echo $TBG_tasks_adjusted_tmp; fi)
...
if [ $node_check_err -eq 0 ] || [ $run_cuda_memtest -eq 0 ] ; then
    ...
    echo "Start PIConGPU."
    test $n_broken_nodes -ne 0 && exclude_nodes="-x./bad_nodes.txt"
    srun -n !TBG_tasks --nodes=!TBG_nodes $exclude_nodes -K1 rocprof --stats !TBG_dstPath/input/bin/picongpu --mpiDirect !TBG_author !TBG_programParams
else
